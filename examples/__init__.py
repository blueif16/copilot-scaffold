...
def load_all_example_tools() -> List[BaseTool]:
    """Auto-discover and load spawn tools from all example apps.

    Each example that has backend spawn tools must export `all_tools` in its `tools.py`.
    """
    ...
    for entry in _iter_examples():
        try:
            ...
            logger.info(f"[examples] loaded {len(tools)} tools from {entry}")
            all_tools.extend(tools)
        except ImportError:
            pass
        except Exception as e:
            logger.warning(f"[examples] error loading tools from {entry}: {e}")
    logger.info(f"[examples] total spawn tools: {len(all_tools)}")
    return all_tools
...
__all__ = ["load_all_example_tools", "load_all_subagents"]
